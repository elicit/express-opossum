...
class CircuitBreaker extends EventEmitter {
  ...
  constructor(next, options = {}) {
    ...
    if (!next) {
      throw new TypeError(
        "No next function provided. Cannot construct a CircuitBreaker without an express NextFunction.",
      );
    }
    ...
    if (this[SHUTDOWN]) {
      ...
      if (
        this.options.state.lastTimerAt !== undefined &&
  ...
  /**
   * Closes the breaker, allowing next to execute again
   * @fires CircuitBreaker#close
   * @returns {void}
   */
  close() {
    if (this[STATE] !== CLOSED) {
      ...
      this[PENDING_CLOSE] = false;
      /**
       * Emitted when the breaker is reset allowing next to execute again
  ...
  open() {
    if (this[STATE] !== OPEN) {
      ...
      this[PENDING_CLOSE] = false;
      /**
       * Emitted when the breaker opens because next has
  ...
  /**
   * Execute next for this circuit. If a fallback function was
  ...
  call(next) {
    ...
    if (!this.closed && !this.pendingClose) {
      ...
      const fallback = fallback(this, error);
      if (fallback) {
        return fallback;
      } else {
        throw error;
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.semaphore.test()) {
        ...
        try {
          ...
          handleError(error, this, timeout, latencyEndTime, resolve, reject);
        }

        if (this.options.timeout && latency > this.options.timeout) {
          timeoutError = true;
          const error = buildError(`Timed out after ${this.options.timeout}ms`, "ETIMEDOUT");
          this.semaphore.release();
          /**
           * Emitted when the circuit breaker next takes longer than
           * `options.timeout`
           * @event CircuitBreaker#timeout
           * @type {Error}
           */
          this.emit("timeout", error, latency);
          handleError(error, this, timeout, latency, resolve, reject);
        ...
        handleError(err, this, timeout, latency, resolve, reject);
      }
    });
  }
...
function handleError(error, circuit, timeout, latency, resolve, reject) {
...
function fallback(circuit, err) {
  if (circuit[FALLBACK_FUNCTION]) {
    try {
      const result = circuit[FALLBACK_FUNCTION].apply(circuit[FALLBACK_FUNCTION], [...err]);
      /**
...
function fail(circuit, err, latency) {
  /**
   * Emitted when the circuit breaker next fails
